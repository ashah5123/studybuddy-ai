      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
            <h3 className="font-semibold text-gray-900">Delete assignment?</h3>
      ...
      <div className={`flex items-start gap-3 py-3 ${completed ? 'opacity-60' : ''}`}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-1.5">
            <span
              className={`text-sm font-medium text-gray-900 ${completed ? 'line-through' : ''}`}
            >
              {assignment.title}
            </span>
            {assignment.type && (
              <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${TYPE_CLASSES[assignment.type] ?? 'bg-gray-100 text-gray-600'}`}>
                {assignment.type}
              </span>
            )}
          </div>
          <div className="mt-0.5 flex items-center gap-2">
            ...
            <span
              className={`text-xs ${overdue && !completed ? 'text-red-600 font-medium' : 'text-gray-400'}`}
            >
              {dueDateText}
            </span>
          </div>
